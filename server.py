...
def serverMains():
    ...
    while (True):
        while ((clientData[0] not in [b'\x30', b'\x31', b'\x32']) or (clientData[1] not in [b'\x30', b'\x31', b'\x32'])):
            pass
        a, b = int(clientData[0].decode()), int(clientData[1].decode())
        if (a == b):
        ...
        clientData[0], clientData[1] = b'', b''

threading.Thread(target=clientGetter1).start()
threading.Thread(target=clientGetter2).start()
serverMains()
